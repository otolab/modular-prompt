...
describe('OpenAIDriver', () => {
  ...
  describe('options.messages', () => {
    it('should append tool result messages to API params', async () => {
      ...
      const calledParams = mockCreate.mock.calls[mockCreate.mock.calls.length - 1][0];
      expect(calledParams.messages).toHaveLength(3); // system + assistant + tool
      ...
      const toolMsg = calledParams.messages[2];
      expect(toolMsg.role).toBe('tool');
      expect(toolMsg.content).toBe('{"temperature":15,"condition":"sunny"}');
    ...
    it('should work correctly when messages is not specified', async () => {
      ...
      const calledParams = mockCreate.mock.calls[mockCreate.mock.calls.length - 1][0];
      // Should only have the system message from prompt
      expect(calledParams.messages).toHaveLength(1);
      expect(calledParams.messages[0].role).toBe('system');
    });
  });
});