...
describe('GoogleGenAIDriver', () => {
  ...
  describe('options.messages', () => {
    ...
    it('should append tool result messages to API contents', async () => {
      const mockGenerateContent = driver['client'].models.generateContent as ReturnType<typeof vi.fn>;
      ...
      const callArgs = mockGenerateContent.mock.calls[mockGenerateContent.mock.calls.length - 1][0];
      expect(callArgs.contents).toHaveLength(3); // system + model + user

      // Check model message with functionCall
      const modelMsg = callArgs.contents[1];
      expect(modelMsg.role).toBe('model');
      expect(modelMsg.parts).toHaveLength(2);
      expect(modelMsg.parts[0]).toEqual({ text: 'Let me check' });
      expect(modelMsg.parts[1].functionCall).toBeDefined();
      expect(modelMsg.parts[1].functionCall.name).toBe('get_weather');
      expect(modelMsg.parts[1].functionCall.args).toEqual({ location: 'Tokyo' });

      // Check user message with functionResponse
      const userMsg = callArgs.contents[2];
      expect(userMsg.role).toBe('user');
      expect(userMsg.parts).toHaveLength(1);
      expect(userMsg.parts[0].functionResponse).toBeDefined();
      expect(userMsg.parts[0].functionResponse.name).toBe('get_weather');
      expect(userMsg.parts[0].functionResponse.response).toEqual({ temp: 15 });
    });
    ...
    it('should work correctly when messages is not specified', async () => {
      ...
      const callArgs = mockGenerateContent.mock.calls[mockGenerateContent.mock.calls.length - 1][0];
      // No additional messages should be appended
      // data/output are empty, so contents should only have default entries
