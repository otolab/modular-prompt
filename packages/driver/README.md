...
ランタイムは `~/.modular-prompt/runtimes/mlx/` に作成されます。状態確認は `pnpm --filter @modular-prompt/driver run runtime:status`（monorepo）または `node node_modules/@modular-prompt/driver/scripts/runtime-cli.js setup --status`。
