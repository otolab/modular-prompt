...
マージ優先（下ほど高）: **`~/.modular-prompt/models.yaml`** → **profile `modelsConfig` overlay**。simple-chat はモデルを同梱しないため、明示的な `-m`、`profile.model`、`workflow.models.default`、または user/profile の `models.default` が必要です。
...
- 会話履歴や資料が多い場合に効果が大きい
- キャッシュファイル（.safetensors.zip）は手動管理（`rm -rf <cacheDir>`でクリア）
