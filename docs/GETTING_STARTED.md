...
// 要約用のユーザーモジュール
// streamProcessingモジュールは内部で自動的にマージされます
const summaryModule = {
...
async function summarizeLongText(text: string) {
  ...
  // streamProcessで実行
  // streamProcessingモジュールは自動的にマージされます
...
// 各アイテムを処理するユーザーモジュール
// concatProcessingモジュールは内部で自動的にマージされます
...
async function answerMultipleQuestions(questions: string[]) {
  ...
  // コンテキストを準備
  const context = {
    chunks: questions.map(q => ({ content: q })),
    results: []
  };

  // concatProcessで実行
  // concatProcessingモジュールは自動的にマージされます
...
import type { ToolSpec, ToolAgentContext } from '@modular-prompt/process';

// コンテキスト型を定義（ToolAgentContext を extends）
interface SearchContext extends ToolAgentContext {
  query: string;
  results?: string[];
}

// ツール定義（AIに渡す定義 + 実行ハンドラー）
// handler は第2引数に context を受け取る
const tools: ToolSpec<SearchContext>[] = [
  {
    definition: {
      name: 'search',
...
const agentModule: PromptModule<SearchContext> = {
  ...
  // DynamicContent で context から動的にプロンプトを構築
  state: [
    (ctx) => ctx.results ? `検索結果: ${ctx.results.length}件` : null,
  ],
};

const result = await toolAgentProcess(driver, agentModule, { query: 'TypeScript' }, {
  tools,
...
console.log(result.context.messages);      // 会話履歴（assistant + tool result）
```

**設計ポイント:**

- **毎ターン re-compile**: ツールが context を変更すると、次のターンで `compile(module, context)` が再実行され、DynamicContent が最新の context を反映する
- **会話履歴は context.messages に蓄積**: `ToolAgentContext` が `messages?: MessageElement[]` を定義し、toolAgentProcess が自動的に assistant メッセージとツール結果を蓄積する
- **コンテキスト操作ツール**: handler が context.messages を直接操作（例: splice で古いメッセージを除去）すれば、次の compile でトークンを節約できる
